...
If the CLI is unavailable but MCP is connected, call `get_marketing_review` directly and use its packet. If the installed server has not shipped this verb yet, use `get_insights` plus publishing/metrics reads, and name the reduced coverage. Use the same explicit dates on every read; never splice a rolling-window delta into a different calendar comparison.
...
After delivering the business brief and report path, follow [keep setup current](../../references/setup-continuation.md#keep-setup-current). Read `next_actions`, verify remaining identity/icon/voice and relevant connection gaps against saved state, and include the next useful integration step alongside the review’s business recommendation. Preserve completed setup and explicit declines; an icon suggestion must never outrank a failed delivery or a consequential review finding. Otherwise follow agent runtime’s account/CLI handoff when the user wants connected results or the weekly email; a local review must still be delivered with its data gaps. Reading/reviewing does not authorize publishing, paid generation, store changes or external writes. If a separately authorized growth change ships, record an `add_event` with its actual `areas` and `why`.
